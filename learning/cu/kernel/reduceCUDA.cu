...
/*
  d_nums is expected to have the following layout:
    ...
    d_nums[3] : DeadlineMs
    d_nums[4] : tasks (total number of nonce iterations to try)
...
__device__ static int globalMutex = 0;

// Lock the mutex: attempt to acquire the lock.
__device__ int lockMutex() {
    ...
    if (atomicCAS((int *)(void *)&globalMutex, 0, 1) != 0) {
		if (atomicCAS((int *)(void *)&globalMutex, 0, 1) != 0) {
			if (atomicCAS((int *)(void *)&globalMutex, 0, 1) != 0) {
				return 1;
			}
...
// Unlock the mutex.
__device__ void unlockMutex() {
    // Memory fence to ensure all previous writes are visible.
    
    atomicExch((int *)&globalMutex, 0);
    
}

__device__ static volatile int exitFlag = 0;
// Must exit.
__device__ int mustExit(int myFlag) {
    // Memory fence to ensure all previous writes are visible.
    
    int ret = atomicAdd((int *)(void *)&exitFlag, 0) < myFlag;
...
__device__ static uint32_t allocator = 0;
...
*/
	...
	if (mustExit(myFlag)) {
        ...
        uint32_t* new_alpha0 = &arena[atomicAdd(&allocator, size0)];
	if (new_alpha0 == 0) {
		goto exit;
	}
        uint32_t* new_alpha1 = &arena[atomicAdd(&allocator, size1)];
	if (new_alpha1 == 0) {
		///free(new_alpha0);
		goto exit;
	}
	...
	if (progOffset+2 >= res * 2) {
        ...
        uint32_t new_maxl = (count0 > count1) ? count0 : count1;
        ...
        atomicExch(&d_nums[0], maxx);
        atomicExch(&d_nums[1], count0);
        atomicExch(&d_nums[2], count1);
...
exit:
	...
	///free0 = 0;
	///free1 = 0;
	atomicExch((int *)(void *)&exitFlag, myFlag+1);
	unlockMutex();
	return;
