...
func (h *HyperParameters) byReducing(alphabet *[2][]uint32) [][2]uint32 {
	...
	for maxmax := maxx; maxx <= maxmax; {
		...
		parallel.Loop(h.Threads).LoopUntil(func(nonce uint32, ender parallel.LoopStopper) (ret bool) {
			// unstucker, see below
			if (initial || maxx == 4 || maxx == 3) && int(nonce) == h.DeadlineMs {
				return true
			}
			...
			if maxx == 2 && maxl == 1 {
				if hash.Hash(alphabet[0][0], centers, maxx) == 1 &&
					hash.Hash(alphabet[1][0], centers, maxx) == 0 {
					return
				}
			} else {
				...
				isvBad := func(v uint32, which uint8) bool {
					...
					//w0 := v / subwords
					//w1 := (v % subwords) << 1
					if (buf[v]>>0)&twobitmask == 0 {
						size++
					}
					buf[v] |= uint32(1+wh) << 0
					return buf[v]&(buf[v]>>1) != 0
				}
	...
	if maxl == 1 && alphabet[0][0] == 0 && alphabet[1][0] == 1 {
		// if some thread is late, create for it a dummy result to append to, take the real result for returning
		program_mut.Lock()
		prog := program
		program = nil
		program_mut.Unlock()
